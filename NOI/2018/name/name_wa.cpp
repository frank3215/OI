...
//	debug(r-l+1, lg[r-l+1]);
	int k = lg[r-l+1];
//	debug(k, l, r-(1<<k)+1, h[k][l], h[k][r-(1<<k)+1]);
...
//	debug(u, cnt[u], ch[u][0], ch[u][1], l, r);
}
int minv(int u, int v, int l, int r, int w) { // min{i>=w}
//	debug(u, v, l, r, w);
	if (cnt[u] == cnt[v]) return inf;
	if (l == r) return l;
	int mid = (l+r)>>1;
	int ret = inf;
	if (mid >= w) ret = minv(ch[u][0], ch[v][0], l, mid, w);
	if (ret != inf) return ret;
	return minv(ch[u][1], ch[v][1], mid+1, r, w);
}
int maxv(int u, int v, int l, int r, int w) { // max{i<=w}
	if (cnt[u] == cnt[v]) return -inf;
	if (l == r) return l;
	int mid = (l+r)>>1;
	int ret = -inf;
	if (mid+1 <= w) ret = maxv(ch[u][1], ch[v][1], mid+1, r, w);
	if (ret != -inf) return ret;
	return maxv(ch[u][0], ch[v][0], l, mid, w);
}
void init() {
	FOR(i,1,pos[1]-1) {
//		debug(i, rnk[i]);
		insert(rt[i] = rt[i-1], 1, pos[T+1]-1, rnk[i]);
	}
}
...
void solve(int n) {
	...
	FOR(i,1,n+1) {
		if (b[sa[i]] > 0) {
//			debug(r[b[sa[i]]], l[b[sa[i]]]-1);
			int previ = maxv(rt[r[b[sa[i]]]], rt[l[b[sa[i]]]-1], 1, pos[T+1]-1, i),
				nexti = minv(rt[r[b[sa[i]]]], rt[l[b[sa[i]]]-1], 1, pos[T+1]-1, i);
			int lcp = query(pre[b[sa[i]]]+1, i);
			int len = pos[b[sa[i]]+1]-sa[i]-1;
//			debug(0, lcp);
			if (nexti != inf) {
//				debug(i+1, nexti, query(i+1, nexti));
				lcp = max(lcp,
						min(query(i+1, nexti),
							r[b[sa[i]]]+1-sa[nexti])
						);
...
//			debug(b[sa[i]], i, sa[i], previ, nexti, lcp, len);
//			if (previ > -inf) debug(sa[previ]);
//			if (nexti < inf) debug(sa[nexti]);
			ans[b[sa[i]]] += max(0,len-lcp);
		}
		if (b[sa[i]] != -1) {
			pre[b[sa[i]]] = i;
		}
...
//	FOR(i,1,pos[T+1]) printf("% 3d", i); putchar('\n');
//	FOR(i,1,pos[T+1]) printf("% 3d", b[i]); putchar('\n');
//	FOR(i,1,pos[T+1]) printf("  %c", s[i]); putchar('\n');
	SA(pos[T+1]-1);
	ST(pos[T+1]-1);
//	FOR(i,1,pos[T+1]) printf("% 3d", sa[i]); putchar('\n');
//	FOR(i,1,pos[T+1]) printf("% 3d", h[0][i]); putchar('\n');
	init();
	solve(pos[T+1]-1);
	FOR(i,1,T+1) {
		W(ans[i]);
