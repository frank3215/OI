...
bool isk;
int _L,_R;
void add(int &u, int uu, int l, int r, int v) {
	u = ++node; sz[u] = sz[uu]+1;
	if (l == r) return;
	int mid = (l+r)>>1;
	if (v <= mid) {
		add(ls[u], ls[uu], l, mid, v);
		rs[u] = rs[uu];
	}
	else {
		add(rs[u], rs[uu], mid+1, r, v);
		ls[u] = ls[uu];
	}
}
/*
...
void query(const int &u, const int &v, int l, int r) { // min{i>=w}
	if (sz[u] == sz[v] || isk) return;
	if (_L <= l && r <= _R) {
		isk += sz[u]-sz[v];
		return;
	}
	int mid = (l+r)>>1;
	if (_L <= mid) query(ls[u], ls[v], l, mid);
	if (mid < _R) query(rs[u], rs[v], mid+1, r);
}
/*
void query(const int&ri,const int&le,int l,int r,int _L,int _R){
	if(sz[ri]==sz[le]||isk)return;
	if(_L<=l&&r<=_R)return(void)(isk=1);
	const int mid=l+r>>1;
	if(_L<=mid)query(ls[ri],ls[le],l,mid,_L,_R);
	if(mid<_R&&!isk)query(rs[ri],rs[le],mid+1,r,_L,_R);
}
*/
